...
- **Reusable scripts** live OUTSIDE the repo in `C:\Users\semaa\framer-extract\` (verify / explore / dump-producer / download-assets + the `*-buyer` variants, official SDK installed there so the repo's `package.json` stays clean). Swap the node id to pull the next page. Node ids: `/`=zQXlyBvQw · `/producer`=xBOUpdJ7Q · `/buyer`=HR8UGJkeJ · `/chat`=xfYr_mapi (next) · `/login`=Wh_r1omb3 · `/inviterequest`=ZXnvgZRVL.
- **`getChildren(pageId)` race (bit us on /buyer):** immediately after `connect(...?node=ID)`, `getChildren(ID)` can return `[]` even with the `?node=` param — the page node isn't hydrated yet. **Prime first:** `await getNodesWithType("WebPageNode")` (and/or `getCanvasRoot()`) before `getChildren`. Tell-tale: `screenshot(ID)` returns a valid image but children are empty → it's this race, not a wrong id.
- **Producer/buyer design was desktop-only** (1241px) → we built the responsive/mobile layout ourselves, **harmonized with Home** (white sections, green accents, existing components — not Framer's literal gray/blue), and added **Inter for body** (Space Grotesk stays for headings via an `h1–h4` base rule + the `font-display` utility). Fixed Framer copy typos (e.g. "bluerberry" → "blueberry"; "+1" → "#1"). **Framer "images" that are really baked text/layout** (the buyer §2 coverage cards, the §8 "Stop Sourcing Blind" comparison) get **rebuilt as live, responsive, accessible markup** (semantic `<table>` w/ `scope` + a stacked `<dl>` for mobile), not shipped as PNGs. Other mockups are trimmed with `sharp().trim()` and floated.
